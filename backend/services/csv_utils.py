...
STATIC_DIR = BASE / "input_data" / "csv" / "static"
...
def find_latest_processed_eod() -> Optional[Path]:
    """Return newest processed_*.csv in PROCESSED_DIR (prefer filename date then mtime)."""
    if not PROCESSED_DIR.exists():
        return None
    candidates = list(PROCESSED_DIR.glob("processed_*.csv"))
    if not candidates:
...
def get_market_snapshot(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    if symbol_cols:
        for sc in symbol_cols:
            try:
                ...
                rows = df[df[sc].astype(str).str.upper() == s]  # keep attempt; defensive
            if not rows.empty:
                break
    ...
    try:
        from services.image_utils import get_logo_path, get_banner_path  # optional helper in your codebase
    except Exception:
...
def load_indices_df(force_reload: bool = False) -> Optional[pd.DataFrame]:
    ...
    candidates = [
        STATIC_DIR / "broad_and_sector_indices.csv",
        STATIC_DIR / "BROAD_AND_SECTOR_INDICES.csv",
        STATIC_DIR / "broad and sector indices.csv",
    ]
    ...
    for p in candidates:
        if p.exists():
            found = p
            break
    if found is None and STATIC_DIR.exists():
        # fallback: any CSV with 'sector' or 'index' in name
        for p in STATIC_DIR.glob("*.csv"):
